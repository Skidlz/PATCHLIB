		...
		unsigned char tonebyte[NPATCH][NCODPARAM])	/* print patch parameters to line printer */
{
	...
	while (1){
		...
		case 'S':
			...
			dispnames(tonebyte);
			writword(BWC,"You can enter up to 4 patch number (12,56,66,41)"
					,0,SCRNTALL-3);

			while (1){
				csrplot(0, SCRNTALL - 2);
				/* writtty(' ',79); */
				writword(BWC,"Enter patch number to print (ret to exit)->"
						,0,SCRNTALL-2);
...
{								/* names stored for edit function */
	...
	for (i = 0; i < pages; i++){
		...
		for (j = 0; j < NUMACROSS; j++){
			/* get name and values for patches */
			if (printall){
				k = (i * NUMACROSS) + j;
